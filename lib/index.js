...
function Pointer(obj) {
	...
	function forwardEvents(fsWatch) {
		...
		['change', 'error'].forEach(function (ev) {
			fsWatch.on(ev, function (arg1, arg2) {
				fsWatch.close();
				self.__stats(true, function (err, _stats) {
					...
					// file changed
					self._stats = utils.clone(_stats);
					eventer.emit('change', stats);
					self.__startWatch();
					return;
				});
			});
...
module.exports.create = function (filePath, stats) {
